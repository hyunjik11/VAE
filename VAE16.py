...
if not data:
    with open(os.path.join("MNIST_data", 'binarized_mnist_train.amat')) as f:
        lines = f.readlines()
        train_data = lines_to_np_array(lines).astype('float16')
    with open(os.path.join("MNIST_data", 'binarized_mnist_valid.amat')) as f:
        lines = f.readlines()
        validation_data = lines_to_np_array(lines).astype('float16')
    with open(os.path.join("MNIST_data", 'binarized_mnist_test.amat')) as f:
        lines = f.readlines()
        test_data = lines_to_np_array(lines).astype('float16')
...
class VariationalAutoencoder(object):
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus, 
        ...
        self.x = tf.placeholder(tf.float16, [None, network_architecture["n_input"]])
    ...
    def _create_network(self):
        ...
        eps = tf.random_normal((tf.shape(self.x)[0], n_z), 0, 1, dtype=tf.float16)
        # z = mu + sigma*epsilon
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2, 
        ...
        all_weights['biases_recog'] = {
            'b1': tf.Variable(tf.zeros([n_hidden_recog_1], dtype=tf.float16)),
            'b2': tf.Variable(tf.zeros([n_hidden_recog_2], dtype=tf.float16)),
            'out_mean': tf.Variable(tf.zeros([n_z], dtype=tf.float16)),
            'out_log_sigma': tf.Variable(tf.zeros([n_z], dtype=tf.float16))}
        ...
        all_weights['biases_gener'] = {
            'b1': tf.Variable(tf.zeros([n_hidden_gener_1], dtype=tf.float16)),
            'b2': tf.Variable(tf.zeros([n_hidden_gener_2], dtype=tf.float16)),
            'out_mean': tf.Variable(tf.zeros([n_input], dtype=tf.float16)),
            'out_log_sigma': tf.Variable(tf.zeros([n_input], dtype=tf.float16))}
        return all_weights
...
print("VAE16 took %s seconds" % (time.time() - start_time))
